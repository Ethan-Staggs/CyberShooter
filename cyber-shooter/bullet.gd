...
func initialize(isPlayerFlipped: bool, isPlayer: String) -> void:
	...
	elif isPlayerFlipped == true and isPlayer == "drone":
		$AnimatedSprite2D.modulate = Color(114,54,0)
		$AnimatedSprite2D.rotation = -95
		speed = -speed
		bulletDirection = -1
		
		
	elif isPlayerFlipped == false and isPlayer == "drone":
		$AnimatedSprite2D.modulate = Color(114,54,0)
		$AnimatedSprite2D.rotation = 95
		speed = speed
		bulletDirection = 1
...
func _process(delta):
	position += Vector2(speed * delta, bulletDirection * speed * delta)
